...
class PaperBook(Book):
    ...
    def __str__(self):
        return f"{super().__str__()}. Страниц в бумажной книге: {self.pages}"
...
class AudioBook(Book):
    ...
    def duration(self, new_duration):
        if not isinstance(new_duration, (int,float)) or new_duration <= 0:
            raise ValueError ("Длительность аудиокниги должна быть числом болше 0")
        self._duration = new_duration


    def __str__(self):
        return f" {super().__str__()}. Продолжительность аудиокниги: {self.duration} ч."
